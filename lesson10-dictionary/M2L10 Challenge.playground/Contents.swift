...
class Library {
    ...
    func searchByTitle(_ title:String) -> String {
        ...
        for (k,v) in catalogue {
            if(v.title == title) {
                inCatalogue = true
                if(checkedOutBooks[k] == nil) {
                    response = "Available"
                } else {
                    response = "Checked out by " + checkedOutBooks[k]!.name
                }
                break
            }
        }
        if(!inCatalogue) {
            response = "Not in catalogue"
        }
    ...
    func checkOut(_ bookId:String, _ person:Person) -> String {
        ...
        var isAvailable = checkedOutBooks[bookId]
        if(isAvailable != nil) {
            response = "Error: Book already checked out"
        } else {
            var found = false
            for (id, book) in catalogue {
                if(id == bookId) {
                    ...
                    checkedOutBooks[bookId] = Person(person.name)
                    break
                }
            }
            if(!found) {
        ...
        for (id, book) in catalogue {
            if(id == bookId) {
                ...
                if(isAvailable != nil) {
                    ...
                    response = "Error: Can't check in a book that hasn't been checked out"
                }
                break
            }
        }
